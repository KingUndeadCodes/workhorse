...
/**
 * POST /api/projects — creates a project. Body: `{ name, key, leadId? }`. 400 if name/key
 * missing, 409 if the key is already taken (issue keys are `${key}-${suffix}`, so two
 ...
 * and seeds a board with one column per that workflow's statuses. Emits `project.created`
 * purely for webhook/automation visibility — the DB write already happened via
 * {@link projectRepo}, mirroring the git-repo-link split (repo CRUD + event emitted alongside).
 */
projectsRouter.post('/projects', async (c) => {
  const body = await c.req.json<{ name: string; key: string; leadId?: string }>();
...
/** PATCH /api/projects/:id — edits name/lead (or archives via `archivedAt`). `key` is intentionally not editable — it's baked into every existing issue's key string. */
projectsRouter.patch('/projects/:id', async (c) => {
  const body = await c.req.json<Partial<Pick<Project, 'name' | 'leadId' | 'archivedAt'>>>();
