...
import { getWorkspace, listCommentsFor, listIssueLinksFor, listIssues, listWatchersFor } from './queries';
...
/**
 * Replays the durable event log and checks it against `state.db` for every issue the log
 * actually describes — status, assignee, sprint, logged time, watchers, links, and comment
 * set, each reconstructed purely from the events that track that field. If these ever
 * disagree, either the projector missed something or a route mutated `state.db` outside
 * the event path — both are bugs this is meant to catch.
 *
 * Deliberately does not attempt to reconstruct or verify issues from the initial seed load:
 * seeding is a bulk data load, not a sequence of commands (see seed.ts), so those issues
 * have no `issue.created` event to replay from and are correctly absent here, not flagged.
 * Only issues created during normal operation — i.e., the ones this rule is actually meant
 * to protect — are checked.
 */
export async function runAudit(): Promise<AuditReport> {
  ...
  function ensure(issueId: string): ExpectedIssueState {
  ...
  for (const [issueId, expected] of expectedByIssue) {
    ...
    const actualWatchers = new Set((await listWatchersFor(issueId)).map((w) => w.userId));
    ...
    const actualLinkIds = new Set((await listIssueLinksFor(issueId)).map((l) => l.id));
    ...
    const actualCommentIds = new Set((await listCommentsFor(issueId)).map((c) => c.id));
